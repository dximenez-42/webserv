...
AConfig::AConfig(std::string config_file)
{
...
AConfig::AConfig(const AConfig &AConfig)
{
	*this = AConfig;
}

AConfig &AConfig::operator=(const AConfig &AConfig)
{
	if (this != &AConfig)
	{
		_servers = AConfig._servers;
	}
	return *this;
}
...
void	AConfig::parseHttp(std::vector<std::string>& split, unsigned int line_number)
{
	if (split.size() != 2)
...
void	AConfig::parseServer(std::vector<std::string>& split, unsigned int line_number)
{
...
void	AConfig::parseRoutes(std::vector<std::string>& split, unsigned int line_number)
{
	if (split.size() != 3)
...
bool	AConfig::checkRouteExists(Route& route)
{
...
const char *AConfig::FileOpenException::what() const throw()
{
	return "Couldn't open file";
}

const char *AConfig::ConfigErrorException::what() const throw()
{
